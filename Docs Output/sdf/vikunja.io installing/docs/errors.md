...
| ErrorCode | HTTP Status Code | Description |
|---|
| 17001 | 400 | The client\_id at token exchange does not match the one used during authorization. |
| 17002 | 400 | The redirect\_uri was rejected or does not match the one used during authorization. |
| 17003 | 400 | Missing code\_challenge or wrong code\_challenge\_method (must be S256). |
...
| 17006 | 400 | The code\_verifier does not match the code\_challenge. |
| 17007 | 400 | Unsupported grant\_type (must be authorization\_code or refresh\_token). |
